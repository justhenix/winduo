...
class Wallpaper {
    std::mutex mutex;std::condition_variable changed;bool stop=false;int command=0;Frame frame;bool normal=false;
    ...
    void apply(const Frame& source,bool strong){
    ...
    void run(){winrt::init_apartment(winrt::apartment_type::multi_threaded);
        for(;;){int work;Frame input;bool strong,ending;{std::unique_lock lock(mutex);changed.wait(lock,[&]{return stop||command;});ending=stop;work=ending?2:command;command=0;input=std::move(frame);strong=normal;}
...
public:
    ...
    void requestApply(Frame source,bool strong){std::lock_guard lock(mutex);frame=std::move(source);normal=strong;command=1;changed.notify_one();}
